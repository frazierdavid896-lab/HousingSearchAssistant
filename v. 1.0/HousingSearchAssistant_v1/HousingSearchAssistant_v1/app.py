...
st.caption("Version 1.1 — parcel-to-facility GIS screening")
            ...
            with st.spinner("Locating parcels and loading GIS layers..."):
                ...
                if points.empty:
                    unresolved_details = "; ".join(
                        f"{row['Address']} — {row['Location Status']}"
                        for _, row in locations.iterrows()
                    )
                    raise RuntimeError(
                        "No property addresses could be located. "
                        + (unresolved_details or "Check the spelling and include city, state, and ZIP code.")
                    )

                properties = attach_parcels(points)
                if properties.empty:
                    raise RuntimeError("Located addresses could not be converted to property geometry.")

                minx, miny, maxx, maxy = properties.total_bounds
                pad = .04
                bounds = (miny - pad, minx - pad, maxy + pad, maxx + pad)
                facilities, warnings = load_all_facilities(bounds, daycare_layer=daycare_layer)
                summary, evidence = screen_properties(properties, facilities, daycare_loaded)
